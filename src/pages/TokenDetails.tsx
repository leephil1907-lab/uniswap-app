...
export default function TokenDetails() {
  ...
  // Real market chart data from 7-day sparkline and live market price feeds
  ...
  const generateChartData = () => {
    ...
    // Historical calculation derived from actual 24h market price change
    const count = timeFrame === '1D' ? 24 : timeFrame === '1W' ? 48 : 96;
    const basePrice = currentPrice || 1.0;
    const changeFactor = (priceChange24h || 0) / 100;
    const startPrice = basePrice / (1 + changeFactor);

    return Array.from({ length: count }).map((_, i) => {
      const fraction = i / (count - 1 || 1);
      const calculatedPrice = startPrice + (basePrice - startPrice) * fraction;
      return {
        time: `${i}:00`,
        price: Number(calculatedPrice.toFixed(4)),
      };
    });
  };
