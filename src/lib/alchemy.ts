...
export function getAlchemyClient(network: Network = Network.ETH_MAINNET): Alchemy {
  ...
  const apiKey = (import.meta as any).env?.VITE_ALCHEMY_API_KEY || 'public';
  const config = {
    apiKey,
    network,
  };
...
export async function fetchUserTransactionHistory(
  ...
  // Fallback to JSON-RPC query via Ethers if Alchemy indexing is delayed
  if (results.length === 0) {
    try {
...
export async function checkAlchemyTransactionReceipt(
  ...
  try {
    ...
    // Fallback using public JSON-RPC on network failure
    try {
