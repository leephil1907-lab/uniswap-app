...
import { Search, Menu, MoreHorizontal, HelpCircle, ChevronDown, ChevronUp, ShieldCheck, ShieldAlert, Sun, Moon, Compass, Rocket, Waves, Wallet, ArrowLeftRight } from 'lucide-react';
...
import { SignatureApprovalModal } from '../common/SignatureApprovalModal';
...
export default function Header() {
  ...
  // Auto-open signature modal when user first connects if not yet verified
  useEffect(() => {
            ...
            <div className="flex items-center gap-2">
              ...
              {!isConnected ? (
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  {/* Signature Approval Status Button */}
                  <button
                    onClick={() => setSignatureModalOpen(true)}
                    className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-bold font-sans transition-all cursor-pointer border ${
                      isAuthenticated
                        ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20'
                        : 'bg-amber-500/15 text-amber-300 border-amber-500/30 hover:bg-amber-500/25 animate-pulse'
                    }`}
                    title={isAuthenticated ? 'Wallet session verified & approved' : 'Signature required to authorize session'}
                  >
                    {isAuthenticated ? (
                      <>
                      ...
                      <>
                        ...
                        <span className="hidden sm:inline">Sign Approval</span>
                      </>
                    )}
                  </button>
